...
loadd(ml_docs)
loadd(big_data_docs)
loadd(governance_docs)
loadd(regulation_docs)
loadd(algorithm_docs)
loadd(ai_ethics_docs)
library(DT)
library(glue)
library(quanteda)
library(eulerr)
...
plot(fit1, quantities = TRUE, legend = TRUE)
```
...
For example, in `ai_ethics_docs` below, "singularity", "sci-fi", "weapons" are significantly more likely to appear in documents that contain both "artificial intelligence" and "ethics/ethical" compared to all other documents. Meanwhile, `ai_no_ethics_docs` - docs that metion AI but do _not_ mention "ethics/ethical", metion "chatbots", "ai-powered", "personalization" etc. far more than the full corpus.
...
top_keys <- keyness_df %>% 
  ...
  filter(row_number(desc(effect_size)) <= 35)
